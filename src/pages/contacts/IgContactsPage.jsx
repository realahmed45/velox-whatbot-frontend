...
export default function IgContactsPage() {
  ...
  const optedInOnPage = contacts.filter((c) => !c.optedOut).length;
      ...
      <div className="relative flex-1 flex flex-col p-4 sm:p-8 overflow-hidden max-w-6xl mx-auto w-full">
        ...
        <div className="mb-6">
          <StatHero
            ...
            stats={[
              ...
              { label: "Tagged (on page)", value: taggedOnPage },
              { label: "Shown", value: contacts.length },
            ]}
          >
            <button
              onClick={exportCSV}
            ...
            <button
              ...
              className="text-xs font-bold px-3.5 py-2.5 rounded-xl bg-white text-brand-700 hover:bg-brand-50 flex items-center gap-1.5 transition shadow-sm"
            >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-5">
          ...
          <div className="relative sm:w-48">
            <TagIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-400" />
            <input
              className="w-full text-sm rounded-xl border border-ink-200 bg-white pl-10 pr-3 py-2.5 text-ink-900 placeholder:text-ink-400 focus:border-brand-400 focus:ring-2 focus:ring-brand-100 focus:outline-none transition"
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="rounded-2xl border border-ink-100 bg-white shadow-sm p-4 animate-pulse"
                >
                  ...
                  <div className="mt-4 h-4 w-24 rounded-full bg-ink-100" />
                </div>
              ))}
            </div>
          ) : contacts.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-20">
              ...
              <p className="font-bold text-ink-900">No followers yet</p>
              <p className="text-sm text-ink-500 mt-1.5 max-w-sm mx-auto leading-relaxed">
                Once people DM your Instagram or trigger a comment automation,
                they'll show up here.
              </p>
              <button
                onClick={() => setShowAdd(true)}
                className="mt-5 text-xs font-bold px-4 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white flex items-center gap-1.5 transition"
              >
                <Plus className="w-3.5 h-3.5" /> Add follower
              </button>
            </div>
              ...
              {contacts.map((c) => {
                ...
                return (
                  <button
                    ...
                    className="group text-left rounded-2xl border border-ink-100 bg-white shadow-sm hover:border-brand-300 hover:shadow-md transition p-4"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1">
                          ...
                          {c.optedOut && (
                            <span className="text-[9px] font-bold text-red-600 bg-red-50 rounded-md px-1.5 py-0.5 flex-shrink-0">
                              OFF
                            </span>
                          )}
                        </div>
                    ...
                    {c.lastTriggerType && (
                      <div className="mt-3 flex items-center gap-1 text-[10px] font-semibold text-brand-700 bg-brand-50 rounded-full px-2 py-1 w-fit">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {(c.tags || []).slice(0, 3).map((t) => (
                        <span
                          key={t}
                          className="text-[10px] font-bold bg-brand-100 text-brand-700 rounded-full px-2 py-0.5"
                        >
                          #{t}
                        </span>
                      ))}
          ...
          {total > 20 && (
            <div className="flex items-center justify-between mt-5 pt-4 border-t border-ink-100">
              ...
              <div className="flex gap-2">
                <button
                  className="text-xs font-bold px-3.5 py-2 rounded-xl border border-ink-200 hover:border-brand-300 disabled:opacity-50 transition"
                  disabled={page === 1}
    ...
    <div className="fixed inset-0 z-40 md:static md:z-auto md:w-96 md:flex-shrink-0">
      ...
      <div className="absolute right-0 inset-y-0 w-full max-w-sm md:static md:max-w-none md:w-full h-full bg-white border-l border-ink-100 flex flex-col shadow-2xl md:shadow-none">
        <div className="h-14 flex items-center justify-between px-5 border-b border-ink-100">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-ink-400 hover:bg-ink-100 hover:text-ink-600 transition"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <button
            onClick={save}
            disabled={saving}
            className="w-full text-sm font-bold py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white disabled:opacity-60 transition"
          >
            ...
            <div className="space-y-2 mb-2 max-h-40 overflow-y-auto">
              ...
              {(contact.notes || [])
                ...
                .map((n, i) => (
                  <div
                    key={i}
                    className="bg-brand-50/60 border border-brand-100 rounded-xl p-2.5 text-xs"
                  >
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="flex gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 text-sm font-bold rounded-xl border border-ink-200 py-2.5 hover:border-brand-300 hover:bg-ink-50 transition"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 text-sm font-bold rounded-xl bg-brand-500 hover:bg-brand-600 text-white py-2.5 disabled:opacity-60 transition"
            >
              {loading ? "Adding…" : "Add"}
            </button>
          </div>
