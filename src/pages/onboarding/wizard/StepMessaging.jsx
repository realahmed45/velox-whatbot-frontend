 ...
 *   Telegram             → GET /channels/telegram/code + poll /status?code=
 */
import { useCallback, useEffect, useState } from "react";
import { Check, Loader2, MessageCircle, Plug } from "lucide-react";
...
export default function StepMessaging({ patch, goNext, goBack }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setStatusMap(map);
      patch({
        messagingConnected: Object.keys(map).filter(
          (k) => map[k]?.status === "connected",
        ),
      });
    } catch {
      /* status is a nicety here — the connect buttons still work */
    } finally {
      setLoading(false);
    }
  ...
  const connect = async (key) => {
    ...
    if (key === "instagram") {
      ...
      if (connected) {
        ...
        load();
      } else if (reason && reason !== "cancelled" && reason !== "redirected") {
        toast.error("Couldn't finish the Instagram connection — try again");
      }
      return;
    }
    setBusy(key);
    try {
      ...
      toast.error(e?.response?.data?.message || "Couldn't start the connection");
      setBusy(null);
    }
  };
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {CHANNELS.map((ch) => {
            ...
            return (
              <div
                key={ch.key}
                className="rounded-2xl border border-ink-100 bg-white p-5 flex flex-col shadow-card"
              >
