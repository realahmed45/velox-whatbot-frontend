 ...
 * Falls back to manual token if OAuth is not configured.
 */
import { useState } from "react";
import { ChevronDown, ChevronUp, Link2, Loader2, ShoppingBag } from "lucide-react";
...
export default function ShopifyConnect({
  ...
  const startOAuth = async () => {
    ...
    try {
      ...
      if (data.fallbackManual) {
        setShowManual(true);
        toast("Use the manual token option below instead.", { icon: "ℹ️" });
        return;
      }
  ...
  const connectManual = async () => {
    const s = manualStore.trim();
    const t = manualToken.trim();
    if (!s || !t) return toast.error("Enter both your store name and access token");
    ...
    try {
      ...
      toast.error(e.response?.data?.message || "Connection failed. Check your store name and token.");
    } finally {
    ...
    <div className={`space-y-3 ${compact ? "" : "border border-ink-200 bg-white p-4"}`}>
      ...
      <div>
        ...
        >
          ...
          Having trouble? Use a manual access token
        </button>
        {showManual && (
          <div className="mt-2 space-y-2 border border-ink-100 bg-ink-50 p-3">
            <p className="text-[10px] text-ink-500">
              In your Shopify admin → <strong>Settings → Apps → Develop apps</strong> → your app → <strong>API credentials</strong> → copy the Admin API access token.
            </p>
            <input
              type="text"
            ...
            >
              {connectingManual ? <Loader2 className="w-4 h-4 animate-spin" /> : <Link2 className="w-4 h-4" />}
              Connect with token
            </button>
          </div>
        )}
